...
void decryptC(char * msg,char * CK)
{
    int i=0;
    int in;
    while(msg[i])
    {
        ...
        cout<<"Encrypting "<<msg[i]<<"With " <<CK[msg[i]-65]<<endl;
#endif
        in=getIndex(msg[i],CK);
        msg[i]=static_cast<char>(in+65);
        i++;
    }
...
int main()
{
    ...
    cout<<"Enter the seed : ";
    cin>>seed;
    decryptC(seed,FIRST_LEVEL_CIPHER);
    //decrypt the seed
    /*NOW GENERATE SECOND LEVEL SEED USING THE
     * DECRYPTED SEED*/
...
#endif
    decryptC(tmpmsg,SECOND_LEVEL_CIPHER);
    //decryptC(tempSeed,FIRST_LEVEL_CIPHER);
    cout<<"\n\nDECRYPTED MESSAGE : "<<tmpmsg<<"\nDECRYPTED SEED : "<<seed;
    return 0;
}
